...
class TodoScreen(Screen):
  def __init__(self, game, events, name):
    ...
    self.GUI_Elements = {}
    self.GUI_identifier = 'Todo'
    self.images_GUI = {}
    self.table = Table.Table(self, 1, 3, anchor = (20,60), col_widths = [10,10,300], align = 'center')

    self.GUI_Table_Header_Anchor = self.table.anchor
    self.GUI_Table_identifier = 'Todo Table'
    self.GUI_Table_radioGroup = 1
    self.GUI_identifier = 'Todo Screen'
  ...
  def InitGUI(self):
    ...
    # gui_cl = self.game.MakeClickable('Complete Todo Table', self.table.rect, double_click_call_back = self.GetSystemFromInsideTable, parent='Complete Todo Table')
  ...
  def UpdateTable(self):
    ...
    todolist = self.game.todoList.todo
    for index in todolist:
      item = todolist[index]
      unsortedIDs.append([index,
                          item['priority'],
                          item['text'],
                         ])
    ...
    if (self.reDraw_GUI):
      for GUI_ID in self.GUI_Elements:
        ...
        if (element.visible) and (element.clickable):
          ...
          elif (element.clickable.parent == 'Complete Todo Table'):
